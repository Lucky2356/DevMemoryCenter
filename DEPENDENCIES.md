...
Redaction, UUID, and other feature dependencies remain deferred until the task that needs them includes a focused comparison. The logging foundation uses only the Rust standard library: its closed event vocabulary, synchronous bounded writer, rotation, retention, and permissions do not justify `tracing` or a background appender yet. `serde_json` is test-only and was already present in the lockfile; it validates emitted JSON without entering production code.
