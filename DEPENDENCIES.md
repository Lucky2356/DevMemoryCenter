...
The first domain crate uses only the Rust standard library. UUID-compatible identifiers are represented as validated 16-byte values because the current task does not generate, parse, or serialize UUID text; a UUID dependency will be reviewed only when an application boundary needs generation or wire/database conversion. Text limits, timestamp ordering, privacy defaults, and the closed project-type vocabulary require no external validation package.

The project path boundary uses only `std::path`, filesystem metadata, and canonicalization. It adds no traversal, glob, watcher, or platform-native dependency and performs no recursive enumeration. Same-user filesystem races remain an operating-system limit and require every later access to use the validated canonical root and repeat relevant checks.
